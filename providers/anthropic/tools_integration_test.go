...
// echoTool is the trivial tool used across the integration tests:
// take a {"text": string}, return it. Enough to prove the round-trip
// (model selects tool, hippo executes, result flows back, model
// references the echoed string in its final response).
...
func TestIntegrationCallWithTools(t *testing.T) {
	...
	var invoked bool
	tool := echoTool{invoked: &invoked}
	ts, _ := hippo.NewToolSet(tool)
	brain, err := hippo.New(hippo.WithProvider(p), hippo.WithTools(ts.All()...))
	if err != nil {
